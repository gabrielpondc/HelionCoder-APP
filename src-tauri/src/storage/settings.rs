...
const HELION_AGENT_ID: &str = "helioncoder";

fn settings_path() -> PathBuf {
    super::data_dir().join("settings.json")
}

fn normalize_agent_id(agent: &str) -> &str {
    match agent {
        "claude" | "codex" => HELION_AGENT_ID,
        other => other,
    }
}
        ...
        match fs::read_to_string(&path) {
            Ok(content) => match serde_json::from_str(&content) {
                Ok(mut settings) => {
                    ...
                    let mut changed = migrate_to_helioncoder(&mut settings);
...
fn migrate_to_helioncoder(settings: &mut AllSettings) -> bool {
    let mut changed = false;

    if settings.user.default_agent != HELION_AGENT_ID {
        log::info!(
            "[storage/settings] migrating default_agent from '{}' to '{}'",
            settings.user.default_agent,
            HELION_AGENT_ID
        );
        settings.user.default_agent = HELION_AGENT_ID.to_string();
        changed = true;
    }

    if !settings.agents.contains_key(HELION_AGENT_ID) {
        let mut migrated = settings
            .agents
            .get("claude")
            .cloned()
            .unwrap_or_else(|| AgentSettings::default_for(HELION_AGENT_ID));
        migrated.agent = HELION_AGENT_ID.to_string();
        migrated.updated_at = crate::models::now_iso();
        settings
            .agents
            .insert(HELION_AGENT_ID.to_string(), migrated);
        changed = true;
    }

    if settings.agents.remove("codex").is_some() {
        changed = true;
    }
    if settings.agents.remove("claude").is_some() {
        changed = true;
    }
...
pub fn update_user_settings(patch: serde_json::Value) -> Result<UserSettings, String> {
    ...
    if let Some(v) = patch.get("display_name") {
        all.user.display_name = if v.is_null() {
            None
        } else {
            v.as_str()
                .map(str::trim)
    ...
    if let Some(agent) = patch.get("default_agent").and_then(|v| v.as_str()) {
        all.user.default_agent = normalize_agent_id(agent).to_string();
    }
    ...
    if let Some(v) = patch.get("helioncoder_cli_path") {
        let old = all.user.helioncoder_cli_path.clone();
        all.user.helioncoder_cli_path = if v.is_null() {
            None
        } else {
            v.as_str()
        ...
        if old != all.user.helioncoder_cli_path {
            crate::agent::claude_stream::invalidate_claude_path_cache();
            log::debug!(
                "[storage/settings] helioncoder_cli_path patched: {:?}",
                all.user.helioncoder_cli_path
            );
        }
    }
