...
fn marketplace_manifest_path(root: &Path) -> PathBuf {
    let preferred = root.join(".helion-plugin").join("marketplace.json");
...
pub fn list_marketplaces() -> Vec<MarketplaceInfo> {
    ...
    for (name, entry) in &entries {
        ...
        let manifest_path = marketplace_manifest_path(&PathBuf::from(&entry.install_location));
...
pub fn list_marketplace_plugins() -> Vec<MarketplacePlugin> {
    ...
    for mp in &marketplaces {
        let manifest_path = marketplace_manifest_path(&PathBuf::from(&mp.install_location));
...
/// List project-level commands from ~/.helioncoder/commands/ and {cwd}/.helioncoder/commands/.
...
pub fn list_project_commands(cwd: &str) -> Vec<crate::models::CliCommand> {
    ...
    if !cwd.is_empty() {
        let project_dir = PathBuf::from(cwd).join(".helioncoder").join("commands");
...
/// List standalone skills from ~/.helioncoder/skills/*/SKILL.md
/// and optionally from {cwd}/.helioncoder/skills/*/SKILL.md.
pub fn list_standalone_skills(cwd: &str) -> Vec<StandaloneSkill> {
    let mut skills = Vec::new();

    // User-scope skills (~/.helioncoder/skills/)
    scan_skills_dir(&skills_dir(), "user", &mut skills);

    // Project-scope skills ({cwd}/.helioncoder/skills/)
    if !cwd.is_empty() {
        let project_dir = PathBuf::from(cwd).join(".helioncoder").join("skills");
...
/// Validates against ~/.helioncoder/skills/, ~/.helioncoder/plugins/, and optionally {cwd}/.helioncoder/skills/.
...
/// Run a `helioncoder plugin ...` CLI command and capture output.
///
...
pub async fn run_plugin_command(
    ...
    cmd.env("PATH", &path_env)
        .env_remove("CLAUDECODE") // Allow running inside a compatible CLI session
...
/// - "user" -> ~/.helioncoder/skills/
/// - "project" -> {cwd}/.helioncoder/skills/
fn resolve_skill_dir(scope: &str, cwd: &str) -> Result<PathBuf, String> {
    match scope {
        ...
        "project" => {
            ...
            Ok(cwd_path.join(".helioncoder").join("skills"))
        }
        _ => Err(format!(
...
/// Allowed: ~/.helioncoder/skills/, ~/.helioncoder/plugins/, or {cwd}/.helioncoder/skills/.
...
fn validate_skill_path(path: &str, cwd: &str) -> Result<PathBuf, String> {
    ...
    if !cwd.is_empty() {
        let project_skills = PathBuf::from(cwd).join(".helioncoder").join("skills");
...
pub fn delete_skill(path: &str, cwd: &str) -> Result<(), String> {
    ...
    // Remove the parent directory (e.g., ~/.helioncoder/skills/my-skill/)
