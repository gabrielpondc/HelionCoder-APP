...
//!
//! Reads HelionCoder CLI transcript files (~/.helioncoder/projects/*/*.jsonl) and converts
//! them into HelionCoder run format (~/.helioncoder/runs/{run-id}/).
...
fn claude_projects_dir() -> Option<PathBuf> {
    super::dirs_next().map(|h| h.join(".helioncoder").join("projects"))
}

/// Validate that a path is within ~/.helioncoder/projects/ (path traversal guard).
fn validate_cli_path(path: &Path) -> Result<(), String> {
    ...
    if let Ok(canonical_projects) = projects_dir.canonicalize() {
        if !canonical.starts_with(&canonical_projects) {
            return Err(format!(
                "path {:?} is outside ~/.helioncoder/projects/",
                canonical
            ));
        }
    }
...
pub fn discover_sessions(target_cwd: &str) -> Result<DiscoverResult, String> {
    ...
    if !projects_dir.exists() {
        log::debug!("[cli_sessions] discover: ~/.helioncoder/projects/ does not exist");
...
pub fn import_session(
    ...
    let meta = RunMeta {
        ...
        prompt: first_prompt,
        cwd: cwd.to_string(),
        agent: crate::agent::claude_stream::HELION_AGENT_ID.to_string(),
        ...
        execution_path: Some(crate::models::ExecutionPath::SessionActor), // CLI import = stream-json session
