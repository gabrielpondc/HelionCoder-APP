...
pub(crate) async fn start_session_impl(
    ...
    let conversation_title = meta
        .name
        .as_deref()
        .filter(|s| !s.trim().is_empty())
        .or(Some(meta.prompt.as_str()));
    adapter::add_session_context(
        &mut adapter_settings,
        user_settings.display_name.as_deref(),
        conversation_title,
    );
    ...
    if remote.is_none() {
        adapter::add_office_skills_context(&mut adapter_settings, meta.app_mode.as_deref());
    }
...
fn side_chat_safe_filename(name: &str) -> String {
...
fn side_chat_extension_for_mime(mime: &str) -> &str {
...
/// Spawn a one-shot pure side-chat CLI process.
/// It intentionally does not resume/fork the current session, so no conversation
/// or workspace context is inherited. Streams text deltas back via Tauri events.
#[tauri::command]
pub async fn side_question(
    ...
    log::debug!(
        "[btw] side_question: run_id={:?}, btw_id={}, question={}, attachments={}",
        run_id,
        btw_id,
    ...
    let source = if let Some(ref id) = run_id {
        Some(storage::runs::get_run(id).ok_or_else(|| format!("Run {} not found", id))?)
    } else {
        None
    };

    // Resolve auth without any remote/workspace target. Chat mode may borrow the
    // source run's provider/model, but never its session_id or cwd.
    let user_settings = storage::settings::get_user_settings();
    let remote: Option<RemoteHost> = None;
    let source_platform = source.as_ref().and_then(|s| s.platform_id.as_deref());
    let effective_platform = if user_settings.auth_mode == "cli" {
        None
    } else {
        platform_id
            .as_deref()
            .or(source_platform)
            .or(user_settings.active_platform_id.as_deref())
    };
    let side_cwd_path = std::env::temp_dir().join("helioncoder-side-chat");
    std::fs::create_dir_all(&side_cwd_path).map_err(|e| e.to_string())?;
    let side_cwd = side_cwd_path.to_string_lossy().to_string();
    let effective_pid = if user_settings.auth_mode == "cli" {
        None
    } else {
        effective_platform
    ...
    let resolved = resolve_auth_env_for_platform(&remote, &user_settings, effective_pid);
    let resolved = augment_with_shell_auth(resolved, &user_settings.auth_mode, false, &side_cwd);

    let mut attachment_lines: Vec<String> = Vec::new();
    let upload_dir = side_cwd_path.join(&btw_id);
    if let Some(atts) = attachments {
        std::fs::create_dir_all(&upload_dir).map_err(|e| e.to_string())?;
        for att in atts.iter().take(8) {
            ...
            let limit = crate::models::max_attachment_size(&att.mime_type) as usize;
            if bytes.len() > limit {
                log::warn!(
                    "[btw] skipping oversized attachment: {} ({} bytes > {} limit)",
                    att.name,
                    bytes.len(),
                    limit
                );
            ...
            let base = side_chat_safe_filename(&att.name);
            let ext = side_chat_extension_for_mime(&att.mime_type);
            let filename = format!(
                "{}-{}-{}{}",
    ...
    let attachment_text = if attachment_lines.is_empty() {
        String::new()
    } else {
        format!(
            "\n\nAttached files uploaded for this side chat:\n{}\nUse only these file paths as local file context.",
    ...
    let wrapped_question = format!(
        "<system-reminder>\n\
You are Helion in a pure side-chat. Do not use or infer the active coding conversation, \
workspace, terminal output, selected files, editor state, or project history. \
Allowed context is only the user's message, files explicitly attached below, and web search/fetch \
if available. Keep the answer helpful and concise. This answer will NOT be added to the main \
conversation history.\n</system-reminder>\n\n{}{}",
    ...
    let agent = source
        ...
        .unwrap_or(claude_stream::HELION_AGENT_ID);
    let agent_settings = storage::settings::get_agent_settings(agent);
    let model_override = model
        .filter(|m| !m.trim().is_empty())
        .or_else(|| source.as_ref().and_then(|s| s.model.clone()));
    let mut adapter =
        adapter::build_adapter_settings(&agent_settings, &user_settings, model_override.clone());
    adapter.allowed_tools = vec!["WebSearch".into(), "WebFetch".into(), "Read".into()];
    adapter.disallowed_tools = vec![
        "Bash".into(),
        "Write".into(),
        "Edit".into(),
        "MultiEdit".into(),
    ...
    adapter.tool_set = Some("WebSearch,WebFetch,Read".into());
    adapter.append_system_prompt = Some(
        "Pure side-chat mode: do not load workspace context. Local file access is only for explicit uploaded attachment paths."
            .into(),
    );
    adapter::clear_model_if_provider_overrides(
        &mut adapter,
