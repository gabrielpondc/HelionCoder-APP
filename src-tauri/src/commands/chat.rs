...
pub async fn send_chat_message(
    ...
    let conversation_title = run
        ...
        .or(Some(run.prompt.as_str()));
    crate::agent::adapter::add_session_context(
        &mut adapter_settings,
        user_settings.display_name.as_deref(),
        conversation_title,
    );
    crate::agent::adapter::add_office_skills_context(
        &mut adapter_settings,
        run.app_mode.as_deref(),
    );
