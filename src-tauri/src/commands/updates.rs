...
pub async fn run_cli_update() -> Result<UpdateCommandResult, String> {
    ...
    let normal_result = run_cli_update_once(&cli_bin, &path_env).await;
    crate::agent::claude_stream::invalidate_claude_path_cache();

    #[cfg(target_os = "macos")]
    {
        match normal_result {
            Ok(result) if !result.success && is_permission_update_failure(&result.output) => {
                log::warn!(
                    "[updates] CLI update failed with permission error; retrying with admin prompt"
                );
                let elevated_result = run_cli_update_with_admin(&cli_bin, &path_env).await;
                crate::agent::claude_stream::invalidate_claude_path_cache();
                return elevated_result.map(merge_cli_update_retry_output);
            }
            Err(err) if is_permission_update_failure(&err) => {
                log::warn!(
                    "[updates] CLI update could not start due to permission error; retrying with admin prompt"
                );
                let elevated_result = run_cli_update_with_admin(&cli_bin, &path_env).await;
                crate::agent::claude_stream::invalidate_claude_path_cache();
                return elevated_result.map(merge_cli_update_retry_output);
...
async fn run_cli_update_once(cli_bin: &str, path_env: &str) -> Result<UpdateCommandResult, String> {
    ...
    run_update_command(
        command,
        "Failed to start CLI updater",
        "CLI updater process error",
    )
    .await
}
...
async fn run_update_command(
    ...
    let child = command
        ...
        .hide_console()
        .spawn()
        .map_err(|e| format!("{}: {}", start_error, e))?;
    ...
    .map_err(|e| format!("{}: {}", process_error, e))?;

    Ok(UpdateCommandResult {
        success: output.status.success(),
        output: update_output_text(&output),
    })
}

fn update_output_text(output: &std::process::Output) -> String {
...
#[cfg(target_os = "macos")]
fn is_permission_update_failure(text: &str) -> bool {
...
#[cfg(target_os = "macos")]
async fn run_cli_update_with_admin(
    cli_bin: &str,
    path_env: &str,
) -> Result<UpdateCommandResult, String> {
    let shell_command = format!(
        "PATH={} HELION_CLI_UPDATE_RELAUNCH=0 HELION_CLI_UPDATE_DETACHED=1 {} update",
        shell_single_quote(path_env),
        shell_single_quote(cli_bin)
    );
    let apple_script = format!(
        "do shell script {} with administrator privileges with prompt {}",
        applescript_string_literal(&shell_command),
        applescript_string_literal("HelionCoder needs administrator permission to update the CLI.")
    );
    ...
    run_update_command(
        command,
        "Failed to request administrator permission for CLI update",
        "CLI administrator updater process error",
    )
    .await
}

#[cfg(target_os = "macos")]
fn merge_cli_update_retry_output(mut result: UpdateCommandResult) -> UpdateCommandResult {
    if result.success && result.output.trim().is_empty() {
        result.output = "CLI update completed with administrator permission.".to_string();
    }
    result
}
