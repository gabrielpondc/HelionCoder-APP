...
fn agents_dir(scope: &str, cwd: Option<&str>) -> Result<PathBuf, String> {
    match scope {
        ...
        "project" => {
            ...
            Ok(cwd_path.join(".helioncoder").join("agents"))
        }
        _ => Err(format!(
...
async fn discover_plugin_agents() -> Vec<AgentDefinitionSummary> {
    ...
    for mp in &marketplaces {
        let manifest_root = PathBuf::from(&mp.install_location);
        let preferred_manifest = manifest_root
            .join(".helion-plugin")
            .join("marketplace.json");
        let manifest_path = if preferred_manifest.is_file() {
            preferred_manifest
        } else {
            manifest_root
                .join(".claude-plugin")
...
pub async fn list_agents(cwd: Option<String>) -> Result<Vec<AgentDefinitionSummary>, String> {
    ...
    // User scope: ~/.helioncoder/agents/
    let user_dir = crate::storage::teams::claude_home_dir().join("agents");
    all.append(&mut scan_agents_dir(&user_dir, "user", "user", false));

    // Project scope: {cwd}/.helioncoder/agents/
    if !cwd_str.is_empty() {
        let project_dir = PathBuf::from(cwd_str).join(".helioncoder").join("agents");
