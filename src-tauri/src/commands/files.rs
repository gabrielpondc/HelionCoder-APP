...
/// the path as-is if neither exist. Sufficient for `~/.helioncoder` / `~/.claude`
...
pub(crate) fn validate_file_path(
    ...
    let raw_requested = PathBuf::from(path);
    ...
    let requested = if raw_requested.is_relative() {
        extra_allowed
            .filter(|base| !base.trim().is_empty())
            .map(|base| PathBuf::from(base).join(&raw_requested))
            .unwrap_or(raw_requested)
    } else {
        raw_requested
    };
