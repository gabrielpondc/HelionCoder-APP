...
(function() {
  ...
  // ── Toolbar (revealed from the bottom edge) ──

  function createToolbar() {
    if (toolbar) return;
    var edge = document.createElement('div');
    edge.id = '__ocv_hotzone';
    edge.style.cssText = 'position:fixed;left:0;right:0;bottom:0;height:14px;z-index:1000000;background:transparent;';
    ...
    bar.style.cssText = BAR + 'display:flex;align-items:center;padding:8px 16px;gap:10px;' +
      'transform:translateY(100%);opacity:0;pointer-events:none;transition:transform .16s ease,opacity .16s ease;';
    bar.innerHTML =
      '<span style="color:#a6adc8;flex:1;font-size:11px;">HelionCoder Preview</span>' +
      '<button id="__ocv_btn_pick" style="' + BTN + 'background:#3b82f6;color:#fff;">Pick Element</button>';
    document.body.appendChild(bar);
    toolbar = bar;

    edge.onmouseenter = function() { setToolbarVisible(true); };
    bar.onmouseenter = function() { setToolbarVisible(true); };
    bar.onmouseleave = function() { if (!active) setToolbarVisible(false); };
