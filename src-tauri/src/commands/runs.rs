...
fn validate_agent_path(agent: &str, _path: &ExecutionPath) -> Result<(), String> {
    match agent {
        "helioncoder" | "claude" => Ok(()), // legacy Claude runs reuse HelionCoder stream-json.
        _ => Err(format!(
            "unknown agent '{}': supported agent is 'helioncoder'",
            agent
        )),
    }
}
...
pub fn start_run(
    ...
    log::debug!(
        "[runs] start_run: agent={}, model={:?}, remote={:?}, platform={:?}, path={:?}, hidden={:?}, prompt_len={}, cwd={}",
        agent,
        model,
        remote_host_name,
        platform_id,
    ...
    let path: ExecutionPath = match execution_path {
        ...
        None => ExecutionPath::SessionActor,
    };
