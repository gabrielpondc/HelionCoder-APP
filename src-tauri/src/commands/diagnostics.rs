...
fn resolved_cli_available(resolved: &str) -> bool {
    ...
    if path.is_absolute() || resolved.contains('/') || resolved.contains('\\') {
        return path.is_file();
    }
...
pub async fn check_agent_cli(agent: String) -> Result<CliCheckResult, String> {
    let normalized_agent = match agent.as_str() {
        "helioncoder" | "claude" => "helioncoder",
    ...
    let version = if let Some(resolved) = path.as_deref() {
        ...
        match ver_output {
            Ok(output) if output.status.success() => {
                ...
                // Strip trailing suffix like " (HelionCoder)" to get bare semver
    ...
    Ok(CliCheckResult {
        agent: normalized_agent.to_string(),
        found,
        path,
        version,
    })
...
fn resolve_openai_resource_url(base_url: &str, resource: &str) -> Result<String, String> {
    ...
    let mut url = url::Url::parse(raw).map_err(|e| format!("Invalid Base URL: {}", e))?;
    let mut path = url.path().trim_end_matches('/').to_string();
    for suffix in ["/responses", "/models", "/chat/completions", "/completions"] {
    ...
    if path.ends_with("/v1") {
        ...
        let next = format!("{}/v1/{}", path, resource).replace("//", "/");
        url.set_path(&next);
    }
    Ok(url.to_string())
}
...
fn parse_model_ids(payload: &serde_json::Value) -> Vec<String> {
    let raw_items = payload.get("data").unwrap_or(payload);
...
pub async fn list_api_models(
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            let error = if e.is_timeout() {
                ...
                "Connection refused while reading models".to_string()
            } else {
    ...
    if !status.is_success() {
        let suffix = if status.as_u16() == 401 {
            "; check the API key"
        } else {
            ""
        };
...
pub async fn test_remote_host(
    ...
    let check_cmd_str = if let Some(claude_bin) = remote_claude_path.as_deref() {
        let escaped_bin = shell_escape(claude_bin);
        format!("command -v {} && {} --version", escaped_bin, escaped_bin)
    } else {
...
async fn check_cli_inner() -> CliDiagnostics {
    ...
    CliDiagnostics {
        ...
        version: cli.version,
        path: cli.path,
        latest: None,              // filled by caller after releases fetch
...
async fn fetch_dist_tags_inner() -> (Option<String>, Option<String>, Option<String>) {
        ...
        .build()
    {
        Ok(c) => c,
        Err(e) => {
            log::warn!("[diagnostics] releases: client build failed: {}", e);
    ...
    let (latest, stable) = match resp {
        Ok(r) if r.status().is_success() => {
            let body: serde_json::Value = match r.json().await {
                Ok(v) => v,
                Err(e) => {
                    log::warn!("[diagnostics] releases: json parse failed: {}", e);
                    return (None, None, None);
                }
            };
            let Some(releases) = body.as_array() else {
                log::warn!("[diagnostics] releases: expected array response");
                return (None, None, None);
            };

            let latest = releases
                .iter()
                .find(|release| !release["draft"].as_bool().unwrap_or(false))
                .and_then(release_tag_version);
            let stable = releases
                .iter()
                .find(|release| {
        ...
        Ok(r) => {
            log::debug!("[diagnostics] releases: HTTP {}", r.status());
            (None, None)
        }
        Err(e) => {
            log::warn!("[diagnostics] releases fetch failed: {}", e);
            (None, None)
        }
    };
...
/// Fetch HelionCoder GitHub release versions.
...
pub fn generate_ssh_key() -> Result<SshKeyInfo, String> {
    ...
    let comment = format!("helioncoder@{}", hostname);
    let aug_path = augmented_path();
