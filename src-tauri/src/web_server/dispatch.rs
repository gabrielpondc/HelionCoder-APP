...
pub async fn dispatch_command(
    ...
    let result = match method {
        ...
        "list_api_models" => {
            let api_key = extract_str(&params, "api_key")?;
            let base_url = extract_str(&params, "base_url")?;
            let result = crate::commands::diagnostics::list_api_models(api_key, base_url).await?;
            serde_json::to_value(result).map_err(|e| e.to_string())
        }
        "test_remote_host" => {
        ...
        "get_helion_usage_stats" => {
            let result = crate::commands::stats::get_helion_usage_stats()?;
            serde_json::to_value(result).map_err(|e| e.to_string())
        }
        "clear_usage_cache" => {
        ...
        "set_cli_api_config" => {
            ...
            let base_url = extract_str(&params, "base_url")?;
            let model = extract_str(&params, "model")?;
            let small_model = extract_str(&params, "small_model")?;
            let model_options: Vec<String> = params
                .get("model_options")
                .and_then(|v| serde_json::from_value(v.clone()).ok())
                .unwrap_or_default();
            crate::commands::onboarding::set_cli_api_config(
                api_key,
                base_url,
                model,
                small_model,
