//! HelionCoder CLI utility functions.
//!
...
/// Shared cache for the resolved HelionCoder binary path.
static HELIONCODER_PATH_CACHE: std::sync::Mutex<Option<String>> = std::sync::Mutex::new(None);

fn configured_helioncoder_path() -> Option<String> {
    if let Ok(path) = std::env::var("HELIONCODER_CLI_PATH") {
    ...
    crate::storage::settings::get_user_settings()
        .helioncoder_cli_path
        .as_deref()
        .map(str::trim)
        .filter(|s| !s.is_empty())
...
pub(crate) fn resolve_helioncoder_path() -> String {
    ...
    let candidates = {
        let mut cands = Vec::new();
        if let Some(ref h) = home {
            cands.push(h.join(".helioncoder").join("bin").join("helion-coder"));
            cands.push(h.join(".helioncoder").join("bin").join("helioncoder"));
            cands.push(h.join(".local").join("bin").join("helion-coder"));
            cands.push(h.join(".local").join("bin").join("helioncoder"));
        }
        cands.push(PathBuf::from("/usr/local/bin/helion-coder"));
        cands.push(PathBuf::from("/usr/local/bin/helioncoder"));
        cands
    };
