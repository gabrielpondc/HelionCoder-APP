...
export async function getHelionUsageStats(): Promise<HelionUsageStats> {
  dbg("api", "getHelionUsageStats");
  return invoke<HelionUsageStats>("get_helion_usage_stats");
}
...
export async function listApiModels(
  apiKey: string,
  baseUrl: string,
): Promise<import("./types").ApiModelListResult> {
  dbg("api", "listApiModels", { baseUrl });
  return invoke<import("./types").ApiModelListResult>("list_api_models", {
    apiKey,
    baseUrl,
  });
}
...
export async function sideQuestion(
  runId: string | null,
  question: string,
  attachments?: Attachment[],
  model?: string,
  ...
  return invoke<string>("side_question", {
    runId,
    question,
...
export async function detectWorkspaceTools(): Promise<WorkspaceTool[]> {
  dbg("api", "detectWorkspaceTools");
  return invoke<WorkspaceTool[]>("detect_workspace_tools");
}

export async function openWorkspaceTool(
  kind: string,
  cwd: string,
  source?: string | null,
): Promise<void> {
  dbg("api", "openWorkspaceTool", { kind, cwd, source });
...
export async function installHelioncoderCli(version?: string): Promise<boolean> {
  dbg("api", "installHelioncoderCli", { version: version ? "specified" : "latest" });
  return invoke<boolean>("install_helioncoder_cli", { version: version || null });
}
...
export async function setCliApiConfig(
  apiKey: string,
  baseUrl: string,
  model: string,
  smallModel: string,
  ...
  dbg("api", "setCliApiConfig", { baseUrl, model, smallModel, modelOptions: modelOptions.length });
  return invoke<void>("set_cli_api_config", {
    apiKey,
    baseUrl,
    model,
    smallModel,
