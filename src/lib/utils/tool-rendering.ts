...
/**
 * Detect if a file path targets a HelionCoder plan file (~/.helioncoder/plans/*.md).
 * Legacy Claude plan paths are also accepted for older imported sessions.
 */
export function isPlanFilePath(filePath: string): boolean {
  if (!filePath) return false;
  const normalized = filePath.replaceAll("\\", "/");
  return (
    (normalized.includes("/.helioncoder/plans/") ||
      normalized.startsWith(".helioncoder/plans/") ||
...
/** Extract the /.helioncoder/plans/<name>.md suffix from a plan file path.
...
export function planFileSuffix(filePath: string): string | null {
  ...
  const normalized = filePath.replaceAll("\\", "/");
  const planRoots = [".helioncoder/plans/", ".claude/plans/"];
  for (const root of planRoots) {
    const absoluteRoot = `/${root}`;
    const idx = normalized.lastIndexOf(absoluteRoot);
    if (idx >= 0 && normalized.endsWith(".md")) return normalized.slice(idx);
    if (normalized.startsWith(root) && normalized.endsWith(".md")) return "/" + normalized;
  }
  return null;
}
...
export function extractPlanContent(
  ...
  for (let i = writeIndex + 1; i < flat.length; i++) {
    ...
    const fp = String(entry.tool.input?.file_path ?? entry.tool.input?.path ?? "");
    if (!isPlanFilePath(fp)) continue;
    // Compare suffix path (/.helioncoder/plans/<name>.md) for cross-format compatibility
