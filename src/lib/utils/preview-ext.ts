...
export const OFFICE_PREVIEW_EXTENSIONS: ReadonlySet<string> = new Set(["docx", "pptx", "xlsx"]);
...
export type PreviewKind = "markdown" | "image" | "office" | "text";
...
export function isOfficePreviewable(ext: string): boolean {
...
export function classifyPath(path: string): PreviewKind {
  ...
  if (isOfficePreviewable(ext)) return "office";
