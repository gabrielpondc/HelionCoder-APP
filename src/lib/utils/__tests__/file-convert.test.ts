...
vi.mock("jszip", () => ({
  default: {
    loadAsync: mocks.jszipLoadAsync,
  },
}));
...
describe("convertFile", () => {
  ...
  describe("unsupported format", () => {
    it("throws for unsupported extension", async () => {
      const file = mockFile("slides.ppt");
      await expect(convertFile(file)).rejects.toThrow("Unsupported conversion format: .ppt");
    });
  });

  describe("pptx conversion", () => {
    it("converts pptx slides to markdown sections", async () => {
      const slide1 = {
        async: vi.fn().mockResolvedValue("<p:sld><a:t>Hello</a:t><a:t>World</a:t></p:sld>"),
      };
      const slide2 = {
        async: vi.fn().mockResolvedValue("<p:sld><a:t>Second &amp; final</a:t></p:sld>"),
      };
      mocks.jszipLoadAsync.mockResolvedValueOnce({
        files: {
      ...
      expect(result.text).toContain("Second & final");
    });

    it("throws on empty presentation", async () => {
      mocks.jszipLoadAsync.mockResolvedValueOnce({ files: {} });

      const file = mockFile("empty.pptx");
      await expect(convertFile(file)).rejects.toThrow("Presentation appears to be empty");
    });
  });
