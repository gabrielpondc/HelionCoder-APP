...
describe("deriveAutoName", () => {
  it("returns the full submitted instruction trimmed", () => {
    expect(deriveAutoName("Hello world\nSecond line")).toBe("Hello world\nSecond line");
  });

  it("does not truncate long instructions", () => {
    const long = "A".repeat(50);
    expect(deriveAutoName(long)).toBe(long);
  });
...
describe("shouldAutoName", () => {
  ...
  it("uses the exact first prompt in autoName", () => {
    const long = "A".repeat(50) + "\nline2";
    const result = shouldAutoName({ ...base, prompt: long });
    expect(result.fire).toBe(true);
    expect(result.autoName).toBe(long);
  });
