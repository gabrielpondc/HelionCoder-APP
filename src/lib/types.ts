...
export interface HelionModelStats {
  model: string;
  sessions: number;
  messages: number;
  inputTokens: number;
...
export interface HelionDailyStats {
  date: string;
  sessions: number;
  messages: number;
  inputTokens: number;
...
export interface HelionUsageStats {
  ...
  byModel: HelionModelStats[];
  daily: HelionDailyStats[];
  sqlitePath: string;
}
...
export interface AuthOverview {
  ...
  /** Source of CLI API key: "settings" | "env:VAR" | "shell_config:PATH" */
