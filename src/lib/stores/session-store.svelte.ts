...
const HELION_AGENT_ID = "helioncoder";

function normalizeAgent(agent: string | undefined): string {
  return agent === "claude" || agent === "codex" || !agent ? HELION_AGENT_ID : agent;
}
...
export class SessionStore {
  ...
  model: string = $state("");
  error: string = $state("");
  agent: string = $state(HELION_AGENT_ID);
  ...
  get useStreamSession(): boolean {
    ...
    return this.agent === HELION_AGENT_ID;
  }
  ...
  async loadRun(
    ...
    try {
      ...
      this.agent = normalizeAgent(this.run.agent);
  ...
  async resumeSession(
    ...
    try {
      ...
      this.run = run;
      this.agent = normalizeAgent(run.agent);
