...
<div bind:this={wrapperEl} class="inline-flex items-center">
  <button
    bind:this={buttonEl}
    class="flex items-center justify-center gap-1 rounded-md {iconOnly
      ? 'h-8 w-8 p-0'
  ...
  >
    ...
    <span class={iconOnly ? "sr-only" : ""}>{t("skillSelector_label")}</span>
    <svg
      class="{iconOnly ? 'hidden' : ''} h-2.5 w-2.5 text-foreground/30"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
