...
<div class="mt-2 space-y-1.5" onclick={(e) => e.stopPropagation()}>
  {#snippet truncateOverlay(isTruncated: boolean)}
    {#if isTruncated && !outputExpanded}
      <div
        class="pointer-events-none absolute bottom-0 inset-x-0 h-8 bg-gradient-to-t from-slate-950/90 to-transparent rounded-b"
      ></div>
    {/if}
  {/snippet}
  {#snippet copyIconButton(action: () => void, className: string)}
    <button
      type="button"
      class={className}
      onclick={action}
      title={copyFeedback ?? t("common_copy")}
      aria-label={copyFeedback ?? t("common_copy")}
    >
      {#if copyFeedback}
        <svg
          class="h-3.5 w-3.5"
    ...
    {#if tool.input?.command}
      ...
      >
        ...
        {#if isInputStreaming || tool.status === "running"}
          ...
          ></span>
        {/if}
        {@render copyIconButton(
          () => handleCopy(`$ ${tool.input?.command}\n${terminalPlainText}`),
          `tool-copy-icon-btn absolute top-1.5 right-1.5 text-neutral-500 hover:text-neutral-300 opacity-0 group-hover/copy:opacity-100 ${copyFeedback ? "is-copied" : ""}`,
        )}
    ...
    {#if filePath}
      <div class="tool-file-header flex items-center justify-between rounded-t">
        ...
        <div class="flex items-center gap-2 shrink-0">
          ...
          {#if readContent}
            {@render copyIconButton(
              () => handleCopy(readContent),
              `tool-copy-icon-btn text-muted-foreground hover:text-foreground ${copyFeedback ? "is-copied" : ""}`,
            )}
          {/if}
        </div>
      </div>
  ...
  {:else if tool.tool_name === "Write" || tool.tool_name === "write_file"}
    ...
    {:else if tool.input?.content}
      ...
      <div class="rounded bg-muted relative group/copy">
        ...
        {@render copyIconButton(
          () => handleCopy(content),
          `tool-copy-icon-btn absolute top-1.5 right-1.5 text-muted-foreground hover:text-foreground opacity-0 group-hover/copy:opacity-100 ${copyFeedback ? "is-copied" : ""}`,
        )}
      </div>
      {#if truncated}
        <button
  ...
  {:else}
    ...
    {#if tool.input && Object.keys(tool.input).length > 0}
      <div class="rounded bg-muted p-2 max-h-40 overflow-y-auto relative group/copy">
        ...
        {@render copyIconButton(
          () => handleCopy(JSON.stringify(tool.input, null, 2)),
          `tool-copy-icon-btn absolute top-1.5 right-1.5 text-muted-foreground hover:text-foreground opacity-0 group-hover/copy:opacity-100 ${copyFeedback ? "is-copied" : ""}`,
        )}
      </div>
    {/if}
