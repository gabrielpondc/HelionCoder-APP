...
{#if open}
  ...
  >
    ...
    >
      ...
      <div class="flex items-center justify-between border-b border-border px-6 py-4">
        <div class="flex items-center gap-3">
          <span class="text-xs text-muted-foreground"
            >{appVersion ? `HelionCoder v${appVersion}` : ""}</span
          >
          <button
      ...
      <!-- Footer -->
      <div class="space-y-2 border-t border-border px-6 py-3 text-xs text-muted-foreground">
        <div class="flex flex-wrap items-center gap-x-4 gap-y-1">
          <span>由 杨凤伟 指导开发</span>
          <span>由 顾家楷 完成开发</span>
          <span>由 叶凌云 提供开发工具</span>
        </div>
        <div class="flex items-center justify-between gap-3">
          <span>Apache License 2.0</span>
          <span>Copyright 2025-2026 HelionCoder Contributors</span>
        </div>
      </div>
    </div>
  </div>
