<script lang="ts">
  ...
  // ── Layout context ──
  type LayoutSidebarContext = { readonly open: boolean; toggle: () => void };
  const layoutSidebar = getContext<LayoutSidebarContext | undefined>("layoutSidebar");
  const toggleLayoutSidebar = layoutSidebar?.toggle ?? getContext<() => void>("toggleSidebar");
  let layoutSidebarOpen = $derived(layoutSidebar?.open ?? true);
  ...
  let helionUsageStats = $state<HelionUsageStats | null>(null);
  let workspaceGitSummary = $state<GitSummary | null>(null);
  let statsTab = $state<"overview" | "models">("overview");
  ...
  function getStoredModelForPlatform(
    ...
    const stored = localStorage.getItem(modelStorageKey(platformId))?.trim();
  ...
  let sidebarRequestedTab = $state<SidebarPanel | null>(null);
  let requestedPreviewPath = $state<string | null>(null);
  let requestedPreviewMode = $state<"preview" | "diff" | null>(null);
  type AppMode = "chat" | "cowork" | "code";

  function requestSidebarTab(tab: SidebarPanel) {
    sidebarCollapsed = true;
    sidebarRequestedTab = tab;
  }
  ...
  const FALLBACK_WORKSPACE_TOOLS: api.WorkspaceTool[] = [
    ...
    { id: "terminal", name: "Terminal", available: true, source: null },
  ];
  const WORKSPACE_TOOL_ORDER = [
    "vscode",
    "vscode-insiders",
    "cursor",
    "windsurf",
  ...
  let promptEditorContextPath = $derived(
    appMode === "chat"
      ? ""
  ...
  let promptCanResume = $derived(
    !store.sessionAlive &&
      canResumeNow(store.run, store.phase, agentSettings?.no_session_persistence ?? false),
  );
  let workspaceLauncherTools = $derived.by(() => {
    const tools = workspaceToolsLoaded ? workspaceTools : FALLBACK_WORKSPACE_TOOLS;
    return [...tools]
      .filter((tool) => tool.available)
      .sort((a, b) => {
        const ai = WORKSPACE_TOOL_ORDER.indexOf(a.id);
        const bi = WORKSPACE_TOOL_ORDER.indexOf(b.id);
        return (ai === -1 ? 999 : ai) - (bi === -1 ? 999 : bi);
      })
      .slice(0, 8);
  });
  let showWorkspaceLauncher = $derived(
    appMode === "code" && !store.isRemote && workspaceLauncherTools.length > 0,
  );
  let promptVisible = $derived(
    store.sessionAlive ||
      ...
      store.phase === "empty" ||
      store.phase === "ready" ||
      TERMINAL_PHASES.includes(store.phase),
  );

  function openPreviewForPath(path: string) {
    if (!path) return;
    requestedPreviewPath = path;
    requestedPreviewMode = "preview";
  ...
  $effect(() => {
    if (store.agent !== "helioncoder" && store.agent !== "claude") return;
  ...
  let workspaceSummaryRequestId = 0;
  async function loadWorkspaceGitSummary(cwd: string) {
    const requestId = ++workspaceSummaryRequestId;
    if (!cwd || store.isRemote) {
      workspaceGitSummary = null;
      return;
    }
    try {
      const summary = await api.getGitSummary(cwd);
      if (requestId !== workspaceSummaryRequestId) return;
      workspaceGitSummary = summary.total_files > 0 ? summary : null;
    } catch (e) {
      if (requestId !== workspaceSummaryRequestId) return;
      workspaceGitSummary = null;
      dbg("chat", "workspace git summary unavailable", { cwd, error: String(e) });
    }
  }
  ...
  $effect(() => {
    ...
    const cwd = promptCwd;
    void store.isRemote;
    loadWorkspaceGitSummary(cwd);
  });

  onMount(() => {
    const handler = () => loadWorkspaceGitSummary(promptCwd);
    window.addEventListener("ocv:cwd-changed", handler);
    window.addEventListener("ocv:project-changed", handler);
    window.addEventListener("ocv:runs-changed", handler);
    return () => {
      window.removeEventListener("ocv:cwd-changed", handler);
      window.removeEventListener("ocv:project-changed", handler);
      window.removeEventListener("ocv:runs-changed", handler);
    };
  });
  ...
  $effect(() => {
    const runIdForCompact = store.run?.id;
    if (!runIdForCompact) return;
    if (!store.useStreamSession || !store.sessionAlive) return;
    if (store.phase !== "idle" || store.isRunning) return;
    if (store.hasInlinePermission || store.hasElicitation) return;
    if (processingSlashCmd || autoCompactInFlight) return;
    if (store.contextWindow <= 0) return;
    if (store.contextWarningLevel !== "critical" && store.contextUtilization < 0.9) return;

    const key = `${runIdForCompact}:${store.compactCount}:${store.numTurns}:${Math.round(
      store.contextUtilization * 100,
    )}`;
    if (key === autoCompactKey) return;
    autoCompactKey = key;
    autoCompactInFlight = true;
    showChatToast(t("chat_autoCompactTriggered"));
    void sendMessage("/compact", []).finally(() => {
  ...
  const codeHeatmapDays = Array.from({ length: 126 }, (_, i) => {
    const active = [
      67, 73, 74, 80, 81, 82, 87, 88, 89, 94, 95, 96, 101, 102, 103, 108, 109, 115,
    ].includes(i);
    return { id: i, active, strong: i % 11 === 0 || i % 17 === 0 };
  });

  let displayName = $derived(settings?.display_name?.trim() || "Helion");

  const welcomeCopy = $derived.by(() => {
    const zh = currentLocale().startsWith("zh");
    if (appMode === "chat") {
      return {
        title: zh ? "想聊点什么？" : "What should we talk about?",
        chips: zh
          ? ["把这段想法整理成清晰表达", "陪我拆解一个问题", "帮我写一封更自然的回复"]
          : [
              "Turn a rough idea into clear wording",
              "Help me think through a problem",
              "Draft a natural reply",
            ],
      };
    }
    if (appMode === "cowork") {
      return {
        title: zh ? "今天要处理什么？" : "What should we get done?",
        chips: zh
          ? ["整理本周会议纪要并生成摘要", "清洗表格并找出异常数据", "把 PDF 内容整理成报告"]
          : [
              "Summarize this week's meeting notes",
              "Clean a spreadsheet and find outliers",
              "Turn a PDF into a report",
            ],
      };
    }
    return {
      title: zh ? "我们该构建什么？" : "What should we build?",
      chips: zh
        ? [
            "审查最近提交的正确性风险和可维护性隐患",
            "帮我处理最近一个未合并 PR 卡住的问题",
            "将常用应用连接到 HelionCoder",
          ]
        : [
            "Review recent commits for correctness risks",
            "Help me fix a stuck pull request",
            "Connect my usual apps to HelionCoder",
          ],
    };
  });
  ...
  const codeWelcomeStats = $derived.by(() => {
    const input = cumulativeTokens.input + cumulativeTokens.cacheRead;
    const output = cumulativeTokens.output + cumulativeTokens.cacheWrite;
    const total = input + output;
    const stats = helionUsageStats;
    return {
      sessions: stats?.totalSessions ?? (store.run ? 1 : 0),
      messages: stats?.totalMessages ?? store.timeline.length,
      tokens: stats
        ? formatTokenCount(stats.totalTokens)
        : total > 0
          ? formatTokenCount(total)
          : "0",
      activeDays: String(stats?.activeDays ?? 0),
      currentStreak: `${stats?.currentStreak ?? 0}d`,
      longestStreak: `${stats?.longestStreak ?? 0}d`,
      peakHour: formatPeakHour(stats?.peakHour),
      favoriteModel: stats?.favoriteModel || store.model || "—",
      codeLines: `+${stats?.totalAdditions ?? 0} / -${stats?.totalDeletions ?? 0}`,
    };
  });

  const codeHeatmap = $derived.by(() => {
    const days = helionUsageStats?.daily ?? [];
    if (days.length === 0) return codeHeatmapDays;
    const recent = days.slice(-126);
    const maxTokens = Math.max(...recent.map((day) => day.totalTokens), 1);
    const padded = Array.from({ length: Math.max(0, 126 - recent.length) }, (_, id) => ({
      id,
      active: false,
      strong: false,
    }));
    return [
      ...padded,
      ...recent.map((day, idx) => ({
        id: padded.length + idx,
        active: day.sessions > 0 || day.totalTokens > 0,
        strong: day.totalTokens / maxTokens > 0.45,
      })),
    ];
  });
  ...
  async function loadHelionUsageStats() {
    try {
      helionUsageStats = await api.getHelionUsageStats();
    } catch (e) {
      dbgWarn("chat", "failed to load Helion usage stats", e);
    }
  }
  ...
  onMount(async () => {
    ...
    try {
      agentSettings = await api.getAgentSettings("helioncoder");
      ...
      if (agentSettings?.effort) {
        api.updateAgentSettings("helioncoder", { effort: "" }).catch(() => {});
      }
    } catch (e) {
  ...
  onMount(() => {
    const handler = () => {
      checkProjectInit();
      if (appMode !== "chat" && !runId && !store.run) {
        const cwd = localStorage.getItem("ocv:project-cwd") || "";
        reloadProjectData(cwd);
      }
    };
  ...
  onMount(() => {
    ...
    appMode = loadAppMode();
    window.addEventListener("helion:mode-change", onModeChange);
    return () => window.removeEventListener("helion:mode-change", onModeChange);
  });
  ...
  onMount(() => {
    ...
    function onProfileChanged() {
      api
        .getUserSettings()
        .then((fresh) => (settings = fresh))
        .catch((e) => dbgWarn("chat", "profile refresh failed", e));
    }
    window.addEventListener("ocv:runs-changed", onRunsChanged);
    window.addEventListener("helion:profile-changed", onProfileChanged);
    return () => {
      window.removeEventListener("ocv:runs-changed", onRunsChanged);
      window.removeEventListener("helion:profile-changed", onProfileChanged);
    };
  });
  ...
  onMount(() => {
    ...
    const onCommandSendPrompt = (e: Event) => {
      const prompt = (e as CustomEvent<{ prompt?: string }>).detail?.prompt;
      if (!prompt) return;
      if (!store.run && prompt.trim().startsWith("/compact")) {
        showChatToast(t("chat_noActiveSession"));
        return;
      }
      void sendMessage(prompt, []);
    };
    const onCommandTogglePlan = () => {
      const nextMode = store.permissionMode === "plan" ? "default" : "plan";
      void handlePermissionModeChange(nextMode);
    };
    const onCommandOpenModel = () => {
      statusBarRef?.openModelDropdown();
    };
    const onCommandExportMarkdown = () => {
      void handleExportMarkdown();
    };
    const onCommandStopRun = () => {
      if (store.run) void handleStop();
      else showChatToast(t("chat_noActiveSession"));
    };
    window.addEventListener("ocv:command-send-prompt", onCommandSendPrompt);
    window.addEventListener("ocv:command-toggle-plan-mode", onCommandTogglePlan);
    window.addEventListener("ocv:command-open-model-selector", onCommandOpenModel);
    window.addEventListener("ocv:command-export-markdown", onCommandExportMarkdown);
    window.addEventListener("ocv:command-stop-run", onCommandStopRun);
    ...
    return () => {
      ...
      window.removeEventListener("ocv:command-send-prompt", onCommandSendPrompt);
      window.removeEventListener("ocv:command-toggle-plan-mode", onCommandTogglePlan);
      window.removeEventListener("ocv:command-open-model-selector", onCommandOpenModel);
      window.removeEventListener("ocv:command-export-markdown", onCommandExportMarkdown);
      window.removeEventListener("ocv:command-stop-run", onCommandStopRun);
  ...
  async function sendMessage(text: string, attachments: Attachment[]) {
    ...
    try {
      if (!store.run) {
        const pureChatMode = appMode === "chat";
  ...
  async function refreshWorkspaceTools() {
    try {
      workspaceTools = await api.detectWorkspaceTools();
    } catch (e) {
      dbgWarn("chat", "failed to detect workspace tools", e);
      workspaceTools = FALLBACK_WORKSPACE_TOOLS;
    } finally {
      workspaceToolsLoaded = true;
    }
  }

  async function openWorkspaceTool(kind: string, source?: string | null) {
    const cwd = promptCwd?.trim();
    if (!cwd) {
      showChatToast(
        currentLocale().startsWith("zh") ? "请先选择工作区" : "Choose a workspace first",
      );
      return;
    }
    try {
      await api.openWorkspaceTool(kind, cwd, source);
    } catch (e) {
      showChatToast(String(e instanceof Error ? e.message : e));
      dbgWarn("chat", "failed to open workspace tool", { kind, cwd, source, error: e });
    }
  }
  ...
  async function handlePermissionModeChange(
    ...
    if (seq === permissionModeChangeSeq) {
      api.updateAgentSettings("helioncoder", { plan_mode: newMode === "plan" }).catch((e) => {
  ...
  async function handleExportMarkdown() {
    if (!store.run) {
      dbgWarn("chat", "handleExportMarkdown: no run");
      showChatToast(t("export_noConversation"));
      return;
    }

    try {
      const md = await api.exportConversation(store.run.id);
      const { save } = await import("@tauri-apps/plugin-dialog");
      const path = await save({
        defaultPath: `conversation-${store.run.id.slice(0, 8)}.md`,
        filters: [{ name: "Markdown", extensions: ["md"] }],
      });
      if (!path) return;
      await api.writeTextFile(path, md);
      showChatToast(t("export_markdownSuccess"));
    } catch (e) {
      dbgWarn("chat", "handleExportMarkdown failed", e);
      showChatToast(t("export_markdownFailed"));
    }
  }
  ...
  async function handleBtwSend(question: string, attachments: Attachment[] = []) {
    dbg("chat", "btwSend", {
    ...
    try {
      const btwId = await api.sideQuestion(
        store.run?.id ?? null,
        question,
        attachments,
...
<div class="flex h-full overflow-hidden bg-background relative">
  ...
  <div class="flex flex-1 flex-col min-w-0 relative">
    ...
    {#if !welcomeVisible}
      <SessionStatusBar
        ...
        onNavigateParent={store.run?.parent_run_id
          ...
          : undefined}
        cwd={store.effectiveCwd}
        onToggleSidebar={layoutSidebarOpen ? toggleLayoutSidebar : undefined}
        mcpServers={store.mcpServers}
        ...
        onToolsClick={sidebarCollapsed ? () => requestSidebarTab("tools") : undefined}
        remoteHostName={store.remoteHostName}
        ...
        workspaceTools={showWorkspaceLauncher ? workspaceLauncherTools : []}
    ...
    {#if mcpPanelOpen && store.mcpServers.length > 0}
      <div class="absolute right-3 top-9 z-30">
        <McpStatusPanel
          runId={store.run?.id ?? ""}
          mcpServers={store.mcpServers}
    ...
    <div class="flex-1 overflow-hidden relative">
      {#if store.useStreamSession}
        ...
        <div
          class="h-full {welcomeVisible ? 'overflow-hidden' : 'overflow-y-auto'}"
        ...
        >
          {#if welcomeVisible}
            ...
            <div class="flex min-h-full items-center justify-center px-8 py-10">
              <div class="w-full max-w-[790px] animate-slide-up">
                <div class="mx-auto flex w-full flex-col items-center text-center">
                  <h1 class="text-[30px] font-semibold leading-tight text-foreground">
                    {welcomeCopy.title}
                  </h1>
                  <div class="relative mt-10 w-full pt-[82px]">
                    <div class="relative">
                      <PromptInput
                        bind:this={promptRef}
                        agent={store.agent}
                        ...
                        hasRun={false}
                        sessionAlive={store.sessionAlive}
                        canResume={promptCanResume}
                        useStreamSession={store.useStreamSession}
                        isRemote={store.isRemote}
                        cliCommands={promptCommands}
                        models={effectiveModels}
                        currentModel={store.model}
                        effort={store.features.effortSelector ? currentEffort : ""}
                        permissionMode={store.permissionMode}
                        cwd={promptCwd}
                        ...
                        onEffortChange={store.features.effortSelector
                          ? handleEffortChange
                          : undefined}
                        ...
                        {appMode}
                        variant="hero"
                        editedSummary={editedFilesSummary}
                        workspaceSummary={promptWorkspaceSummary}
                        editorContextPath={promptEditorContextPath}
                        onOpenEditedFiles={openEditedFilesPanel}
                        onReviewChanges={() => openDiffForPath()}
                        onRestoreStash={() => {
                          if (stashedInput) {
                            promptRef?.restoreSnapshot(stashedInput);
                            stashedInput = null;
                            showChatToast(t("toast_stashRestored"));
                            dbg("chat", "stash restored via hero badge click");
                          }
                        }}
                      />
                    </div>
                  ...
                  >
                    {#each welcomeCopy.chips as suggestion, i}
                      <button
                        type="button"
                        class="group flex w-full items-center gap-3 px-3 py-3 transition-colors hover:bg-accent/45 hover:text-foreground"
                      ...
                      >
                        ...
                        >
                          ...
                          {:else if i === 1}
                            <path d="M12 3v3" /><path d="M12 18v3" /><path d="M3 12h3" />
                            <path d="M18 12h3" /><path d="m5.6 5.6 2.1 2.1" />
                            <path d="m16.3 16.3 2.1 2.1" />
                            <path d="m18.4 5.6-2.1 2.1" />
                            <path d="m7.7 16.3-2.1 2.1" />
                          {:else}
                            <path d="M7 7h10v10H7z" /><path d="M4 4h4" />
                            <path d="M16 4h4" /><path d="M4 20h4" /><path d="M16 20h4" />
                          {/if}
                        </svg>
                  ...
                  >
                    ...
                    {#if entry.kind === "user"}
                      <ChatMessage
                        ...
                        userDisplayName={settings?.display_name?.trim() || undefined}
                    ...
                    {:else if entry.kind === "command_output"}
                      <div class="w-full py-2">
                        <div class="chat-content-width pl-7">
                          <div
                            class="command-output rounded-lg border border-border/40 bg-muted/50 px-4 py-3 text-sm overflow-x-auto dark:bg-[#1a1b26]"
                          >
            ...
            {#if !store.run}
              <div class="flex flex-col gap-2">
                {#each examplePrompts as prompt}
                  <button
                    class="rounded-lg border border-border/60 bg-muted/30 px-4 py-3 text-left text-sm text-foreground hover:bg-accent/60 hover:border-border transition-all duration-150 group"
    ...
    {#if btwState.active}
      ...
      >
        <div class="flex items-center justify-between px-4 py-2 border-b border-border/50">
          <span class="text-xs font-medium text-blue-400">{t("sideChat_title")}</span>
          <button
            onclick={() => (btwState = { ...btwState, active: false })}
            title={t("sideChat_close")}
            class="text-muted-foreground hover:text-foreground transition-colors"
          >
            <svg
              class="h-3.5 w-3.5"
        ...
        <div class="px-4 py-3 space-y-2">
          <p class="text-xs text-muted-foreground">{t("sideChat_question")}: {btwState.question}</p>
          <div class="text-sm">
            {#if btwState.error}
    ...
    {#if promptVisible && !welcomeVisible}
      <PromptInput
        bind:this={promptRef}
        agent={store.agent}
        running={store.isActivelyRunning}
